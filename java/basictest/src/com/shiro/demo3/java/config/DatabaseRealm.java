package com.shiro.demo3.java.config;
...
public class DatabaseRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        //将密码按照规则进行加密
        User user = userService.queryUserList(new User(userName)).get(0);
        String passwordEncoded = new SimpleHash("md5", password, user.getSalt(), 2).toString();
        //然后和数据库里面的进行比对
        List<User> users = userService.queryUserList(new User(userName, passwordEncoded));
        //若不匹配
        if(users.isEmpty()){
        ...
        //放入的password 要与login方法传入的password一致 不然会直接返回登录失败
